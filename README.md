...
RDP 会话录像以 `.guac` 格式存储在 MinIO 中，key 为 `<session-key>.guac`。所有 RDP 录像统一转换为 MP4 后播放。
...
由 guac-worker 服务执行：`.guac` → guacenc → `.m4v` → ffmpeg (H.264) → `.mp4` → 上传 S3

转换过程中前端展示当前步骤和进度百分比，步骤包括：下载录制文件、编码、转码、上传。
