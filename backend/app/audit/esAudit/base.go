...
func (b *Base) WriteData(data map[string]any) {
	...
	if !elasticsearch.IsExistsIndex(b.Index) {
		if err := elasticsearch.CreateIndex(b.Index); err != nil {
			logger.Error(fmt.Sprintf("创建索引失败-%s", err.Error()))
			return
		}
		if err := elasticsearch.CreateMap(b.Index, b.Mappings); err != nil {
			logger.Error(fmt.Sprintf("创建mapping失败-%s", err.Error()))
			return
		}
	}
	if err := elasticsearch.InsertData(b.Index, data); err != nil {
